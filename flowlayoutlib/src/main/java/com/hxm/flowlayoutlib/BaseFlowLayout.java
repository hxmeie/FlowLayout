...
public class BaseFlowLayout extends ViewGroup {
    ...
    private void init(Context context, AttributeSet attrs) {
        TypedArray array = context.obtainStyledAttributes(attrs,
                R.styleable.BaseFlowLayout);
        mChildHorizontalSpacing = array.getDimensionPixelSize(
                R.styleable.BaseFlowLayout_horizontalSpacing, 0);
        mChildVerticalSpacing = array.getDimensionPixelSize(
                R.styleable.BaseFlowLayout_verticalSpacing, 0);
        mGravity = array.getInteger(R.styleable.BaseFlowLayout_android_gravity, Gravity.LEFT);
        int maxLines = array.getInt(R.styleable.BaseFlowLayout_android_maxLines, -1);
        if (maxLines >= 0) {
            setMaxLines(maxLines);
        }
        int maxNumber = array.getInt(R.styleable.BaseFlowLayout_maxNum, -1);
        if (maxNumber >= 0) {
